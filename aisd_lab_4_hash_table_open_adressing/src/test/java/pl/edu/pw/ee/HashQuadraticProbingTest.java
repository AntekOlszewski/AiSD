...
public class HashQuadraticProbingTest {
    ...
    public void should_ThrowException_WhenInitialSizeIsLowerThanOne() {
        ...
        // when
        HashTable<Double> hash = new HashQuadraticProbing<>(initialSize, 0.5, 0.5);
    ...
    public void searchAfterResizeTest() {
        HashTable<Integer> hash = new HashQuadraticProbing<>(2, 0.5, 0.5);
        hash.put(1);
        hash.put(11);
        hash.put(111);
        int output = hash.get(11);
    ...
    public void timeTest() throws IOException {
        ...
        for (int k = 0; k < 10; k++) {
            System.out.println("a = b = " + (k + 1) * 0.1);
            for (int i = 0; i < 10; i++) {
                List<Long> insertTimes = new ArrayList<>();
                List<Long> searchTimes = new ArrayList<>();
                for (int j = 0; j < 30; j++) {
                    HashTable<String> hashTable = new HashQuadraticProbing<>(512 * (int) Math.pow(2, i), (k + 1) * 0.1,
                            (k + 1) * 0.1);
